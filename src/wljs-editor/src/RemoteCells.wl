...
evaluateNotebook[uid_, kernel_, originNotebook_] := With[{
    notebook = nb`HashMap[uid],
    promise = Promise[]
},
{
    ...
    With[{
        initCells = {Select[Select[notebook["Cells"], cell`InputCellQ], (#["Props"]["InitGroup"] === True) &], FirstCase[notebook["Cells"] // Reverse, _?cell`InputCellQ]} // Flatten, 
        generated = StringReplace[RandomWord[]<>ToString[RandomInteger[{0,9999}] ]<>"g`", {"-"->""}]
    },
        
        Echo["lastInput cell"]; Echo[lastInput];
        ...
        With[
                    ...
                    With[{results = (initCells // Last)["Result"]},
                        If[Length[results] > 0,
                            ...
                            ,
                                ...
                                Echo[">> "<>ToString[Last[results]["Display"], InputForm] ];
                                Echo[">> "<>ToString[Last[results]["Data"], InputForm] ];
                            ]
            ...
            cell`EvaluateCellObj[#, "Evaluator"->kernel["Container"] ] &/@ initCells;
            ...
            Echo["Evaluating notebook..."];
                With[{ref = assoc["Ref"]},
                        If[ !MissingQ[nb`HashMap[ref] ] , 
                            Echo["EvaluateNotebook >> Warning: no reference notebook; evaluation context is lost"];
                        ];
            
                            With[{},
                                Then[evaluateNotebook[hash, kernel, nb`HashMap[ref] ], Function[result, 
           ...
           With[{promise = data["Promise"], kernel = GenericKernel`HashMap[ data["Kernel"] ]},
                ...
                With[{ref = data["Ref"]},
                        If[ !MissingQ[cell`HashMap[ref] ] ,
                            With[{dir = If[MemberQ[cell`HashMap[ref]["Notebook"]["Properties"], "WorkingDirectory"],
                                    (cell`HashMap[ref]["Notebook"]["WorkingDirectory"])
                                ,
                                    If[DirectoryQ[#], #, DirectoryName[#] ] &@ (cell`HashMap[ref]["Notebook"]["Path"])
                                ]
                            },
                                If[StringQ[dir],
            ...
            With[{reference = cell`HashMap[ t["Ref"] ], evaluatedQ = Lookup[t["Meta"], "EvaluatedQ", True], title = Lookup[t["Meta"], "Title", "Projector"], imageSize=Lookup[t["Meta"], ImageSize, Automatic], display = Lookup[t["Meta"], "Display", "codemirror"], target = Lookup[t["Meta"], "Target", "Notebook"]},
                ...
                If[!MatchQ[reference, _cell`CellObj], 
                    With[{
                        ...
                        If[!MatchQ[notebook, _nb`NotebookObj], Return[] ];
                ...
                If[MatchQ[target, "Notebook" | Null | Automatic],
                    With[{c = cell`CellObj @@ Join[{"Notebook" -> reference["Notebook"], "Data" -> t["Data"], "Display" -> display}, 
                        ReplaceAll[ 
                            Normal[KeyDrop[t["Meta"], {"Notebook", "Window"}] ] 
                        , {CoffeeLiqueur`Extensions`RemoteCells`RemoteCellObj -> cell`HashMap}] 
                    ]},
                ...
                ,
                    With[{controller = reference["Notebook"]["Controller"]},
                        EventFire[controller, "NotebookCellDataProject", <|
                            "Cell" -> reference,
                            "Hash" -> t["Meta", "Hash"],
                            "Data" -> t["Data"],
